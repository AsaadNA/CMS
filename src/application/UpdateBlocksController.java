...
public class UpdateBlocksController implements Initializable {
	...
	private void updateTimeBox_ADD(String block) {
		...
		ResultSet result = LoginController.getSQL().executeQuery("SELECT timestampid,timestamp from TIMESTAMPS");
		try {
			while(result.next()) {
				timestamp_hashMap.put(result.getInt(2),result.getInt(1));
				timeSlotList.add(Integer.toString(result.getInt(2)));
			}
		...
		try {
			while(result.next()) {
				timeSlotList.remove(Integer.toString(result.getInt(1)));
			}
		} catch (SQLException e1) {
	...
	private void updateTimeBox_OTHER (String block) {
		...
		try {
			while(result.next()) { tempList.add(Integer.toString(result.getInt(1))); }
